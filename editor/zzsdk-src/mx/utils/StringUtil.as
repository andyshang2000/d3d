...
package mx.utils
{
	...
	public class StringUtil
	{
		...
		public static function substituteExWStyle(str:String, context:Object):String
		{
			if (str == null)
				return '';
			...
			if (context is Class)
			{
				var typeXml:XML = describeType(context);
				var childNode:XML;
				var name:String;
				for each (childNode in typeXml.accessor.(@type == "String"))
				{
					name = childNode.@name;
					if (name.charAt(0) == "_")
					{
					...
					str = str.replace(new RegExp("\\$_" + name + "_", "g"), context[name] || "");
				}
				for each (childNode in typeXml.variable.(@type == "String"))
				{
					name = childNode.@name;
					if (name.charAt(0) == "_")
					{
			...
			else
			{
				...
				{
					str = str.replace(new RegExp("\\$_" + key + "_", "g"), context[key]);
				}
			}
			return str;
		}

		public static function substituteEx(str:String, context:Object):String
		{
			if (str == null)
				return '';
			...
			if (context is Class)
			{
				...
				for each (childNode in typeXml.accessor.(@type == "String"))
				{
					...
					str = str.replace(new RegExp("\\$\\{" + name + "\\}", "g"), context[name] || "");
				}
				for each (childNode in typeXml.variable.(@type == "String"))
				{
					name = childNode.@name;
		...
		public static function pack(param0:String):String
		{
			if (/^(['"]).*\1$/.test(param0))
			{
				return param0;
		...
		public static function unpack(param0:String):String
		{
			if (/^(['"]).*\1$/.test(param0))
			{
				param0 = param0.substring(1, param0.length - 1);
			}
			return param0;
		}
	}
