...
package com.popchan.sugar.modules.game.view
{
    ...
    public class Candy extends Element 
    {
        ...
        public function set color(_arg_1:int):void
        {
            ...
            this.img = new Image(Core.texturesManager.getTexture(_local_2));
        ...
        public function runAsPath():void
        {
            ...
            var _local_4:Object = {
                "time":0.1,
                "y":(this.path[(_local_3 - 1)].pos.y - 3),
        ...
        public function setSpecialStatus(_arg_1:int, _arg_2:Boolean=false, _arg_3:Boolean=false):void
        {
            ...
            this.img = new Image(Texture.fromTexture(Core.texturesManager.getTexture(("candy" + (this.color - 1)))));
            this._status = _arg_1;
            if (_arg_1 == CandySpecialStatus.HORIZ)
            {
                this.img = new Image(Texture.fromTexture(Core.texturesManager.getTexture((("candy" + (this.color - 1)) + "a"))));
            }
            else
            {
                if (_arg_1 == CandySpecialStatus.VERT)
                {
                    this.img = new Image(Texture.fromTexture(Core.texturesManager.getTexture((("candy" + (this.color - 1)) + "b"))));
                }
                else
                {
                    if (_arg_1 == CandySpecialStatus.BOMB)
                    {
                        this.img = new Image(Texture.fromTexture(Core.texturesManager.getTexture(("candybomb" + (this.color - 1)))));
                    }
                    else
                    {
                        if (_arg_1 == CandySpecialStatus.COLORFUL)
                        {
                            this.img = new Image(Texture.fromTexture(Core.texturesManager.getTexture("candyking")));
                        };
                    };
                };
            };
        ...
        public function setBomb(_arg_1:int):void
        {
            ...
            this.img = new Image(Texture.fromTexture(Core.texturesManager.getTexture(("candytimer" + (this.color - 1)))));
            this.img.pivotX = (this.img.width >> 1);
            this.img.pivotY = (this.img.height >> 1);
            this.addChild(this.img);
            ...
            if (!this.bomb_txt)
            {
                this.bomb_txt = ToolKit.createTextSprite(this, Core.texturesManager.getTextures("bombtxt_"), -22, 0, 8, "0123456789", 24);
            };
